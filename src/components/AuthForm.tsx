...
const AuthForm = ({type}: {type: FormType}) => {
  ...
  return (
    <div className="card-border lg:min-w-[566px] my-6">
        <div className="flex flex-col gap-6 card py-14 px-10">
        <div className="flex flex-row gap-2 justify-center">
            <Image src="/logo.svg" alt="logo" width={38} height={32}/>
            <h2 className="text-primary-100">Pinterview</h2>
        </div>
        <h3>Interview Kerja Bersama AI</h3>
       ...
       <FormField control={form.control} 
        ...
        <Button className="btn" type="submit">{isSignIn ? 'Masuk' : 'Buat' }</Button>
      </form>
    </Form>

    <p className="text-center">
    {isSignIn ? 'Belum punya akun?' : 'Sudah punya akun?'}
    <Link href={!isSignIn ? '/sign-in' : '/sign-up'} className="text-user-primary ml-1 font-bold">
